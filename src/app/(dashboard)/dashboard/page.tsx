  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      <div>
        ...
        <p className="text-muted-foreground">Real-time telemetry and excellence metrics across all sites.</p>
      </div>
        ...
        <Card className="lg:col-span-3">
          ...
          <CardContent>
            <div className="space-y-4">
              {ALERTS.map((alert) => (
                <div key={alert.id} className="flex items-start gap-4 rounded-lg border p-3 hover:bg-secondary/50 transition-colors">
                  <div className={cn(
      ...
      <Card>
        <CardHeader>
          <CardTitle>Active Programs Portfolio</CardTitle>
          <CardDescription>Status summary of ongoing R&D and NPI initiatives.</CardDescription>
        </CardHeader>
        <CardContent>
          <div className="space-y-4">
              ...
              <div key={program.id} className="flex items-center justify-between rounded-lg border p-4">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">{program.id} • {program.manager}</p>
                  </div>
                </div>
                <div className="flex items-center gap-8 text-right">
                  <div className="hidden sm:block">
                    <p className="text-xs text-muted-foreground uppercase font-bold mb-1">Phase</p>
                    <Badge variant="outline">{program.phase}</Badge>
                  </div>
                  <div className="w-24">
                    <p className="text-xs text-muted-foreground uppercase font-bold mb-1">Completion</p>
                    <div className="h-2 w-full bg-secondary rounded-full overflow-hidden">
                      <div className="h-full bg-primary" style={{ width: `${program.completion}%` }} />
                    </div>
                    <span className="text-xs mt-1 block">{program.completion}%</span>
                  </div>
                </div>
              </div>
            ))}
